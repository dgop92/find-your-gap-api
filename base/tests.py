...
from base.urls import register_view_name, results_view_name
...
class TestResults(TestsMixin, TestCase):
    ...
    def test_postive_results(self):
        ...
        self.post(self.results_url, data=data, status_code=status.HTTP_200_OK)

    def test_results_without_sd(self):

        data = {"usernames": ["my_user_1", "my_user_2"], "compute_sd": False}

        self.post(self.results_url, data=data, status_code=status.HTTP_200_OK)

    def test_results_with_limit(self):

        data = {"usernames": ["my_user_1", "my_user_2"], "limit": 5}
    ...
    def common_negative_test_generator(self, data):
        ...
        self.post(self.results_url, data=data, status_code=status.HTTP_400_BAD_REQUEST)

    def test_invalid_user(self):

        data = {"usernames": ["my_user_1", "my_user_2", "im bob", "willyrex"]}
...
class TestGeUser(TestsMixin, TestCase):
    ...
    def test_get_user(self):
        ...
        get_user_url = reverse(views.UninorteUserDetail.name, args=["my_user_1"])
        self.get(get_user_url, status_code=200)
        self.compare_json_response_given_data(self.user1_data)

        get_user_url = reverse(views.UninorteUserDetail.name, args=["my_user_2"])
        self.get(get_user_url, status_code=200)
        self.compare_json_response_given_data(self.user2_data)

        get_user_url = reverse(views.UninorteUserDetail.name, args=["my_user_10"])
