...
from base.constants import (BIT_MATRIX_DATA_TYPE, DAYS, DAYS_PER_WEEK, HOURS,
                            HOURS_PER_DAY, UNINORTE_SCHEDULE_SIZE)
...
class DistanceMatrixComputer:
    ...
    def compute_avg(self):
        self.avg_matrix = self.sum_matrix * \
            (1 / len(self.distance_matrices))

    
    def compute_sd(self):

        self.deviation_matrix = np.zeros(
            shape = UNINORTE_SCHEDULE_SIZE, 
            dtype = 'float32'
        )
...
class GapFinder:
    ...
    def __init__(self, string_schedules, compute_sd = False):
        ...
        self.distance_matrix_computer = DistanceMatrixComputer(
            self.distance_computer.get_distance_matrices(),
            self.compute_sd
        )
    ...
    def apply_filters(self, limit = None):
        ...
        if self.compute_sd:
            sort_func = lambda gap: (gap["avg"], gap["sd"])
        else:
            sort_func = lambda gap: gap["avg"]
